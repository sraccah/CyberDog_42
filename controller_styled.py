...
class input_test(object):
            ...
            try:
                ...
                if filename:
                    self.font = pygame.font.Font(
                        filename, self.default.fontsize)
    ...
    def pre_render_circle_image(self):
        ...
        for i in range(size // 2, -1, -1):
            color = (
                lightcolor[0] + i *
                    (basecolor[0] - lightcolor[0]) // (size // 2),
                lightcolor[1] + i *
                    (basecolor[1] - lightcolor[1]) // (size // 2),
                lightcolor[2] + i *
                    (basecolor[2] - lightcolor[2]) // (size // 2),
                255
            )
            pygame.draw.circle(
                self.circle,
    ...
    def init(self):
        ...
        rec_height = max(
            5 + joy.numaxes + joy.numballs +
            joy.numhats + (joy.numbuttons + 9) // 10
            for joy in self.joy
        ) * self.fontheight
        rec_width = max(
    ...
    def draw_hat(self, value, pos):
        ...
        self.screen.fill((127, 127, 127, 255),
                         (left, top + height // 2, width, 1))
        self.screen.fill((127, 127, 127, 255),
                         (left, top + height - 1, width, 1))
        self.screen.fill((127, 127, 127, 255), (left, top, 1, height))
        self.screen.fill((127, 127, 127, 255),
                         (left + width // 2, top, 1, height))
        self.screen.fill((127, 127, 127, 255),
    ...
    def draw_joy(self, joyid):
        ...
        for i, v in enumerate(joy.hat):
            self.draw_hat(
                ...
                (
                    pos[0],
                    pos[1] + (4 + joy.numaxes + joy.numballs + i) *
                    self.fontheight,
                    tmpwidth,
                    self.fontheight
                )
